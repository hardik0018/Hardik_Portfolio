...
export interface ShuffleSegment {
  text: string;
  className?: string;
}
...
export interface ShuffleProps {
  ...
  shuffleTimes?: number;
  animationMode?: 'random' | 'evenodd' | 'sequential'; 
  loop?: boolean;
  loopDelay?: number;
  stagger?: number;
...
const Shuffle = forwardRef<HTMLElement, ShuffleProps>(
  (
    ...
    useEffect(() => {
      ...
      if ('fonts' in document) {
        if (document.fonts.status === 'loaded') setFontsLoaded(true);
          ...
          // Fallback: don't wait more than 1s for fonts
          timeoutId = setTimeout(() => setFontsLoaded(true), 1000);
        }
    ...
    useGSAP(
      () => {
        if (!internalRef.current || (!text && !segments) || !fontsLoaded) return;
        if (
        ...
        const play = () => {
          ...
          const addTween = (targets: HTMLElement[], at: number) => {
            const vars: any = {
              ...
              ease,
              force3D: true,
              stagger: animationMode !== 'random' ? stagger : 0,
            };
            if (isVertical) {
            ...
            if (colorFrom && colorTo) {
              tl.to(
                targets,
                { color: colorTo, duration, ease, stagger: animationMode !== 'random' ? stagger : 0 },
                at
              );
            }
          };
        ...
        const create = () => {
          ...
          // Only auto-play if not in hover-only mode or if triggered by view
          if (!triggerOnHover || triggerOnView) {
            play();
          }
    ...
    const baseTw = 'inline-flex flex-wrap w-fit will-change-transform text-foreground';
    const userHasFont = useMemo(() => className && /font[-[]/i.test(className), [className]);
    const fallbackFont = useMemo(
      () => (userHasFont ? {} : { fontFamily: '"Euclid Square", var(--font-display), Archivo, sans-serif' }),
      [userHasFont]
    );
    ...
    return React.createElement(
      Tag,
      { ref: internalRef as any, className: classes, style: commonStyle },
      contentNodes
    );
  }
);
