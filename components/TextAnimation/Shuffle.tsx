...
export interface ShuffleProps {
  ...
  shuffleTimes?: number;
  animationMode?: 'random' | 'evenodd' | 'sequential'; // Added sequential
  loop?: boolean;
  loopDelay?: number;
  stagger?: number;
...
const Shuffle: React.FC<ShuffleProps> = ({
  ...
  onShuffleComplete,
  shuffleTimes = 1,
  animationMode = 'sequential', // Set sequential as default for "one by one"
  loop = false,
  loopDelay = 0,
  stagger = 0.05, // Increased default slightly for better "one by one" feel
  scrambleCharset = '',
  colorFrom = '',
  colorTo = '',
  triggerOnce = true,
  ...
  const fallbackFont = useMemo(
    () => (userHasFont ? {} : { fontFamily: `'HelveticaNeue', Helvetica, Arial, sans-serif` }),
    [userHasFont]
  );
