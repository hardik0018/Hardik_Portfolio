...
export default function EmergingImage({
  ...
  useEffect(() => {
    ...
    if (fillColor.startsWith("var(")) {
      const varName = fillColor.slice(4, -1).trim();
      const computedValue = getComputedStyle(document.body).getPropertyValue(varName).trim();
      if (computedValue) {
        actualFillColor = computedValue;
      }
    }
    material.uFillColor = new THREE.Color(actualFillColor);
    material.uType = type;
    ...
    loader.load(
      ...
      undefined,
      (err) => {
        console.error("EmergingImage: Error loading texture", url, err);
      }
    );
