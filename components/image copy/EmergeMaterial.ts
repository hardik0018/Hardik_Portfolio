...
const EmergeMaterial = shaderMaterial(
  ...
  /*glsl*/ `
    ...
    void main() {
      ...
      // first transition (pixelize slide-in)
      if(uType==0.){
        // pixelize
        ...
        // third transition (pixel dissolve)
        float s = 120.;
      ...
      // Apply rounded corners (only when radius > 0)
      if(uBorderRadius > 0.0) {
        vec2 pixelPos = vUv * uElementSize;
        vec2 centerPos = pixelPos - uElementSize * 0.5;
        float dist = roundedBoxSDF(centerPos, uElementSize * 0.5, uBorderRadius);
        float smoothEdge = 1.0 - smoothstep(-1.0, 1.0, dist);
        gl_FragColor.a *= smoothEdge;
